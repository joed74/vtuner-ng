...
t_satip_vtuner *satip_vtuner_new(char *devname, char *delsys, char *caids[VTUNER_MAX_SLOTS], char *sids[VTUNER_MAX_SLOTS], t_satip_config *satip_cfg)
{
  ...
  if (delsys != NULL)
  {
    ...
    while (token != NULL)
    {
      if (!strcasecmp(token, "DVBS"))
        vt_delsys.value[i++] = SYS_DVBS;
      if (!strcasecmp(token, "DVBS2"))
        vt_delsys.value[i++] = SYS_DVBS2;
      if (!strcasecmp(token, "DVBC"))
        vt_delsys.value[i++] = SYS_DVBC_ANNEX_A;
      if (!strcasecmp(token, "DVBC_A"))
        vt_delsys.value[i++] = SYS_DVBC_ANNEX_A;
      if (!strcasecmp(token, "DVBC_B"))
        vt_delsys.value[i++] = SYS_DVBC_ANNEX_B;
      if (!strcasecmp(token, "DVBC_C"))
        vt_delsys.value[i++] = SYS_DVBC_ANNEX_C;
      if (!strcasecmp(token, "DVBT"))
        vt_delsys.value[i++] = SYS_DVBT;
      if (!strcasecmp(token, "DVBT2"))
        vt_delsys.value[i++] = SYS_DVBT2;
      token = strtok(NULL, ",");
      if (i > VTUNER_MAX_DELSYS)
        break;
    }
...
static void set_pidlist(struct satip_vtuner *vt, struct vtuner_message *msg)
{
  ...
  for (i = 0; i < MAX_PIDTAB_LEN; i++)
    ...
    {
      INFO(MSG_NET, "got PMT: %i\n", msg->body.pidlist[i] & 0x1FFF);
      // TODO: ADD HERE SOMETHING?
      satip_clear_pmt(vt->satip_cfg);
      satip_add_pmt(vt->satip_cfg, msg->body.pidlist[i] & 0x1FFF);
